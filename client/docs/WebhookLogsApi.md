...
------------- | ------------- | -------------
[**GetOrganizationWebhookLogs**](WebhookLogsApi.md#GetOrganizationWebhookLogs) | **Get** /organizations/{organizationId}/webhookLogs | Return the log of webhook POSTs sent
...
func main() {
    ...
    resp, r, err := apiClient.WebhookLogsApi.GetOrganizationWebhookLogs(context.Background(), organizationId).T0(t0).T1(t1).Timespan(timespan).PerPage(perPage).StartingAfter(startingAfter).EndingBefore(endingBefore).Url(url).Execute()
    if err != nil {
        fmt.Fprintf(os.Stderr, "Error when calling `WebhookLogsApi.GetOrganizationWebhookLogs``: %v\n", err)
        fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
    }
    // response from `GetOrganizationWebhookLogs`: []map[string]interface{}
    fmt.Fprintf(os.Stdout, "Response from `WebhookLogsApi.GetOrganizationWebhookLogs`: %v\n", resp)
}
```
